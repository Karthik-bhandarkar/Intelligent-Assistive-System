...
def speak(text, lang_code='en'):
    ...
    Writes temp files to the OS temp directory to avoid polluting the project tree.
    """
    if not text:
        return
    ...
    def _generate_and_play():
        try:
            # Write to OS temp dir, not CWD, so file is never git-tracked
            filename = os.path.join(
                tempfile.gettempdir(),
                f"tts_{int(time.time())}_{threading.get_ident()}.mp3"
            )
